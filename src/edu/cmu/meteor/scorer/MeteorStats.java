...
public class MeteorStats {
	...
	public double[] toIntArray() {
		double[] stats = new double[STATS_LENGTH];
		int idx = 0;
	 ...
	 * @param stats
	 */
	public MeteorStats(double[] stats) {
		int idx = 0;
		...
		for (int i = 0; i < Constants.MAX_MODULES; i++) {
			...
			double tstC = stats[idx++] / SCALE_FACTOR;
			double refC = stats[idx++] / SCALE_FACTOR;
			...
			double tstF = stats[idx++] / SCALE_FACTOR;
			double refF = stats[idx++] / SCALE_FACTOR;
